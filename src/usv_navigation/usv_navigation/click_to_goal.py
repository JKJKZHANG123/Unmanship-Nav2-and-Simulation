...
class ClickToGoal(Node):
    ...
    def __init__(self):
        ...
        # Monotonic goal generation.  Every queue/cancel bumps it, and each
        # in-flight goal captures the value it was sent under.  A late action
        # result callback from a superseded goal must not clear the *new* goal's
        # handle or republish a stale terminal status.
    ...
    def _on_goal_response(self, future, generation: int):
        try:
            ...
            return
        if generation != self._goal_generation:
            # A cancel or a newer goal superseded this request before Nav2 even
            # acknowledged it.  Drop the stale handle without touching state.
            if handle.accepted:
                try:
        ...
        result_future.add_done_callback(
            lambda result_future: self._on_goal_result(
                result_future, generation, handle))

    def _on_goal_result(self, future, generation: int, handle):
        # Clear the active handle only if it is still the goal we are watching.
        # A superseded goal's late result must not clear the newer goal's handle.
        if self._active_goal_handle is handle:
            self._active_goal_handle = None
        if generation != self._goal_generation:
            # Superseded goal's terminal status; do not republish it.
            return
        try:
