...
def generate_launch_description():
    ...
    enable_goal_gateway = LaunchConfiguration(
        'enable_goal_gateway', default='false')
    dynamic_tracker_params_file = LaunchConfiguration(
        'dynamic_tracker_params_file',
        default=PathJoinSubstitution([
            pkg_share, 'config', 'dynamic_obstacle_tracker.yaml']))
    goal_guard_params_file = LaunchConfiguration(
        'goal_guard_params_file',
        default=PathJoinSubstitution([
    ...
    ld.add_action(DeclareLaunchArgument(
        'enable_goal_gateway',
        default_value='false',
        description='Enable NavigateToPose goal gateway; disable for path-only '
                    'operation. Default false (safe on the real boat); the VRX '
                    'sim scripts pass :=true explicitly.'))
    ld.add_action(DeclareLaunchArgument(
        'dynamic_tracker_params_file',
        default_value=PathJoinSubstitution([
            pkg_share, 'config', 'dynamic_obstacle_tracker.yaml']),
        description='Dynamic obstacle tracker parameter file'))
    ld.add_action(DeclareLaunchArgument(
        'goal_guard_params_file',
        default_value=PathJoinSubstitution([
            pkg_share, 'config', 'goal_guard.yaml']),
        description='Goal guard parameter file'))
    ...
    ld.add_action(Node(
        ...
        output='screen',
        parameters=[dynamic_tracker_params_file],
    ...
    ld.add_action(Node(
        ...
        name='click_to_goal',
        output='screen',
        parameters=[goal_guard_params_file],
        condition=IfCondition(enable_goal_gateway),
    ))
