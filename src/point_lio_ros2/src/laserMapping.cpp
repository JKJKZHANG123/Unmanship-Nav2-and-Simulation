...
struct ImuStationaritySample {
    double stamp;
    double acc_norm;
    double gyro_norm;
};

deque<ImuStationaritySample> zupt_imu_window;
bool zupt_stationary = false;
uint64_t zupt_update_count = 0;
double zupt_last_acc_norm = 0.0;
double zupt_last_gyro_norm = 0.0;
V3D zupt_anchor_position = Zero3d;
bool zupt_anchor_initialized = false;
ofstream zupt_log;

void reset_zupt_detector() {
    zupt_imu_window.clear();
    zupt_stationary = false;
    zupt_last_acc_norm = 0.0;
    zupt_last_gyro_norm = 0.0;
    zupt_anchor_initialized = false;
}

void update_zupt_detector(double stamp, const V3D &acc, const V3D &gyro) {
    if (!zupt_enable) return;

    zupt_last_acc_norm = acc.norm();
    zupt_last_gyro_norm = gyro.norm();
    if (!std::isfinite(stamp) || !std::isfinite(zupt_last_acc_norm) ||
        !std::isfinite(zupt_last_gyro_norm)) {
        zupt_stationary = false;
        zupt_imu_window.clear();
        return;
    }

    if (!zupt_imu_window.empty() && stamp - zupt_imu_window.back().stamp > 0.25) {
        zupt_imu_window.clear();
    }
    zupt_imu_window.push_back({stamp, zupt_last_acc_norm, zupt_last_gyro_norm});
    while (static_cast<int>(zupt_imu_window.size()) > std::max(3, zupt_window_size)) {
        zupt_imu_window.pop_front();
    }
    if (zupt_imu_window.size() < 3) {
        zupt_stationary = false;
        return;
    }
    ...
    for (const auto &sample : zupt_imu_window) {
        acc_mean += sample.acc_norm;
        gyro_mean += sample.gyro_norm;
    }
    acc_mean /= static_cast<double>(zupt_imu_window.size());
    gyro_mean /= static_cast<double>(zupt_imu_window.size());

    double acc_var = 0.0;
    double gyro_var = 0.0;
    for (const auto &sample : zupt_imu_window) {
        acc_var += (sample.acc_norm - acc_mean) * (sample.acc_norm - acc_mean);
        gyro_var += (sample.gyro_norm - gyro_mean) * (sample.gyro_norm - gyro_mean);
    }
    acc_var /= static_cast<double>(zupt_imu_window.size());
    gyro_var /= static_cast<double>(zupt_imu_window.size());

    const double window_duration = zupt_imu_window.back().stamp - zupt_imu_window.front().stamp;
    zupt_stationary = window_duration >= zupt_min_stationary_time &&
                      std::abs(acc_mean - G_m_s2) <= zupt_acc_threshold &&
                      gyro_mean <= zupt_gyro_threshold &&
                      std::sqrt(acc_var) <= zupt_acc_std_threshold &&
                      std::sqrt(gyro_var) <= zupt_gyro_std_threshold;
}

void write_zupt_metrics(double stamp, bool update_applied) {
    if (!zupt_log) return;
    V3D pos = use_imu_as_input ? kf_input.x_.pos : kf_output.x_.pos;
    V3D vel = use_imu_as_input ? kf_input.x_.vel : kf_output.x_.vel;
    zupt_log << std::setprecision(12) << stamp << ','
             << zupt_last_acc_norm << ',' << zupt_last_gyro_norm << ','
             << (zupt_stationary ? 1 : 0) << ',' << zupt_update_count << ','
             << (update_applied ? 1 : 0) << ','
             << pos(0) << ',' << pos(1) << ',' << pos(2) << ','
             << vel(0) << ',' << vel(1) << ',' << vel(2) << '\n';
    zupt_log.flush();
...
bool apply_zupt(double stamp) {
    const bool eligible = zupt_enable && zupt_stationary && imu_en && !is_first_frame &&
                          p_imu && !p_imu->imu_need_init_;
    if (!eligible) {
        // Keep a per-LiDAR diagnostic row even when the correction is not
        // eligible. This makes false negatives (or initialization stalls)
        // visible in MATLAB instead of producing an empty CSV.
        write_zupt_metrics(stamp, false);
        return false;
    }

    const Eigen::Matrix<double, 3, 3> velocity_cov =
        Eigen::Matrix<double, 3, 3>::Identity() * zupt_velocity_cov;
    bool updated = false;
    if (use_imu_as_input) {
        updated = kf_input.update_zero_velocity(V3D::Zero(), velocity_cov);
        if (updated && zupt_position_hold_enable) {
            if (!zupt_anchor_initialized) {
                zupt_anchor_position = kf_input.x_.pos;
                zupt_anchor_initialized = true;
            }
            const Eigen::Matrix<double, 3, 3> position_cov =
                Eigen::Matrix<double, 3, 3>::Identity() * zupt_position_cov;
            updated = kf_input.update_position_anchor(zupt_anchor_position, position_cov) && updated;
        }
    } else {
        updated = kf_output.update_zero_velocity(V3D::Zero(), velocity_cov);
        if (updated && zupt_position_hold_enable) {
            if (!zupt_anchor_initialized) {
                zupt_anchor_position = kf_output.x_.pos;
                zupt_anchor_initialized = true;
            }
            const Eigen::Matrix<double, 3, 3> position_cov =
                Eigen::Matrix<double, 3, 3>::Identity() * zupt_position_cov;
            updated = kf_output.update_position_anchor(zupt_anchor_position, position_cov) && updated;
        }
    }
    if (updated) ++zupt_update_count;
    write_zupt_metrics(stamp, updated);
    return updated;
}
...
void imu_cbk(const sensor_msgs::msg::Imu::SharedPtr msg_in) {
    ...
    update_zupt_detector(timestamp, acc, gyro);
...
int main(int argc, char **argv) {
    ...
    if (zupt_enable) {
        zupt_log.open(DEBUG_FILE_DIR("zupt_metrics.csv"), ios::out);
        if (zupt_log) {
            zupt_log << "time_s,imu_acc_norm_mps2,imu_gyro_norm_rps,stationary_flag,"
                        "zupt_update_count,update_applied,pos_x_m,pos_y_m,pos_z_m,"
...
//------------------------------------------------------------------------------------------------------
    ...
    while (rclcpp::ok()) {
        ...
        if (sync_packages(Measures)) {
            ...
            // These vectors are indexed below, so elements must be constructed.
            // reserve() alone leaves size()==0 and caused memory corruption/SIGSEGV.
            ...
            if (!use_imu_as_input) {
                ...
                for (k = 0; k < time_seq.size(); k++) {
                    ...
                    if (publish_odometry_without_downsample) {
                        // Apply ZUPT before publishing so the corrected state is
                        // visible immediately on /aft_mapped_to_init.
                        apply_zupt(Measures.lidar_last_time);
                        /******* Publish odometry *******/
