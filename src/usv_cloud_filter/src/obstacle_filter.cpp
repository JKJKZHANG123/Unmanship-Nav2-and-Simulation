...
public:
  ...
  : Node("obstacle_filter")
  {
    ...
    // The self mask can be selected as either the legacy WAM-V geometry or a
    // measured rectangular real-boat envelope.  The rectangle is evaluated
    // after transforming each cloud into output_frame (normally base_link).
    ...
    self_y_min_ = declare_parameter<double>("self_y_min", -1.6);
    self_y_max_ = declare_parameter<double>("self_y_max", 1.6);
    self_y_half_width_ = declare_parameter<double>("self_y_half_width", 1.6);
    lidar_mount_height_m_ = declare_parameter<double>("lidar_mount_height_m", 0.0);
    ...
    // Keep the measured real-boat mask and filtering thresholds adjustable from
    // the Qt monitor while the node is running.  The callback updates the
    // cached values used by the next scan; topics, TF frames, and publishers
    // remain unchanged, so a parameter edit cannot silently rewire the graph.
    parameter_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&ObstacleFilter::on_set_parameters, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(
      get_logger(), "  self mask: %s x[%.3f, %.3f] y[%.3f, %.3f], L1 height %.3f m",
      self_mask_mode_.c_str(), self_x_min_, self_x_max_, self_y_min_,
      self_y_max_, lidar_mount_height_m_);
  }

private:
  // Return true for points belonging to the vessel itself.  For a real boat,
  // the configured rectangle is expressed in output_frame (normally
  // base_link), with x forward and y to port/left.  The raw cloud has already
  // been transformed into that frame before this function is called.
  rcl_interfaces::msg::SetParametersResult on_set_parameters(
    const std::vector<rclcpp::Parameter> & parameters)
  {
    // Stage all edits first.  If a GUI request contains an invalid bound,
    // reject the complete transaction without partially changing the active
    // filter configuration.
    auto mode = self_mask_mode_;
    auto x_min = self_x_min_;
    auto x_max = self_x_max_;
    auto y_min = self_y_min_;
    auto y_max = self_y_max_;
    auto raw_min = raw_z_min_;
    auto raw_max = raw_z_max_;
    auto structure_min = structure_z_min_;
    auto structure_max = structure_z_max_;
    auto max_range = max_range_;
    auto voxel_size = voxel_size_;
    auto enable_outlier = enable_outlier_;
    auto outlier_radius = outlier_radius_;
    auto outlier_neighbors = outlier_min_neighbors_;

    for (const auto & parameter : parameters) {
      const auto & name = parameter.get_name();
      if (name == "self_mask_mode") {
        mode = parameter.as_string();
      } else if (name == "self_x_min") {
        x_min = parameter.as_double();
      } else if (name == "self_x_max") {
        x_max = parameter.as_double();
      } else if (name == "self_y_min") {
        y_min = parameter.as_double();
      } else if (name == "self_y_max") {
        y_max = parameter.as_double();
      } else if (name == "raw_z_min") {
        raw_min = parameter.as_double();
      } else if (name == "raw_z_max") {
        raw_max = parameter.as_double();
      } else if (name == "structure_z_min") {
        structure_min = parameter.as_double();
      } else if (name == "structure_z_max") {
        structure_max = parameter.as_double();
      } else if (name == "max_range") {
        max_range = parameter.as_double();
      } else if (name == "voxel_size") {
        voxel_size = parameter.as_double();
      } else if (name == "enable_outlier") {
        enable_outlier = parameter.as_bool();
      } else if (name == "outlier_radius") {
        outlier_radius = parameter.as_double();
      } else if (name == "outlier_min_neighbors") {
        outlier_neighbors = parameter.as_int();
      }
    }

    rcl_interfaces::msg::SetParametersResult result;
    result.successful = false;
    if (mode != "rectangle" && mode != "wamv_geometry") {
      result.reason = "self_mask_mode must be rectangle or wamv_geometry";
      return result;
    }
    if (x_min > x_max || y_min > y_max || raw_min > raw_max ||
      structure_min > structure_max || max_range < 0.0 || voxel_size < 0.0 ||
      outlier_radius <= 0.0 || outlier_neighbors < 1)
    {
      result.reason = "invalid filter bounds or value";
      return result;
    }

    self_mask_mode_ = mode;
    self_x_min_ = x_min;
    self_x_max_ = x_max;
    self_y_min_ = y_min;
    self_y_max_ = y_max;
    raw_z_min_ = raw_min;
    raw_z_max_ = raw_max;
    structure_z_min_ = structure_min;
    structure_z_max_ = structure_max;
    max_range_ = max_range;
    voxel_size_ = voxel_size;
    enable_outlier_ = enable_outlier;
    outlier_radius_ = outlier_radius;
    outlier_min_neighbors_ = outlier_neighbors;
    result.successful = true;
    result.reason = "filter parameters updated for subsequent scans";
    return result;
  }
  ...
  bool is_self_return(const PointT & pt) const
  {
    if (self_mask_mode_ != "wamv_geometry") {
      return pt.x >= self_x_min_ && pt.x <= self_x_max_ &&
             pt.y >= self_y_min_ && pt.y <= self_y_max_;
    }
  ...
  double max_range_;
  double raw_z_min_, raw_z_max_, self_x_min_, self_x_max_, self_y_min_, self_y_max_,
    self_y_half_width_, lidar_mount_height_m_;
