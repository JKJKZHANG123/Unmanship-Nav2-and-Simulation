...
: Node("unitree_lidar_sdk_node", options)
{
  ...
  const int init_result = lsdk_->initialize(
    cloud_scan_num_, port_, 2000000, rotate_yaw_bias_,
    range_scale_, range_bias_, range_max_, range_min_);
  if (init_result != 0) {
    throw std::runtime_error(
            "Failed to initialize Unitree L1 on " + port_ +
            " (SDK error " + std::to_string(init_result) + ")");
  }

  RCLCPP_INFO(
    this->get_logger(), "Initialized Unitree L1 on %s at 2000000 baud", port_.c_str());

  // Follow the vendor SDK startup sequence. A lidar left in standby mode
  // exposes a valid serial port but publishes no point-cloud or IMU data.
  RCLCPP_INFO(this->get_logger(), "Switching Unitree L1 to NORMAL mode");
