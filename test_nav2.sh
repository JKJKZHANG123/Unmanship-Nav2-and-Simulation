...
setsid bash -c "set +u; source /opt/ros/jazzy/setup.bash; source '$SCRIPT_DIR/install/setup.bash'; exec ros2 launch usv_navigation navigation.launch.py use_sim_time:=true enable_goal_gateway:=true" \
  >"$LOG_FILE" 2>&1 &
NAV_PID=$!
