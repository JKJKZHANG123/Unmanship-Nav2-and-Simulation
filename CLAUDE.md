...
Only `usv_navigation` has real unit tests — all pure algorithm functions, no ROS graph:
`test_dynamic_obstacle_tracker.py`, `test_geodesy.py`, `test_mavlink_rtk_bridge.py`,
`test_target_waypoint_serial.py`. Run a single case with `install/setup.bash` sourced:
...
For the **dual-antenna RTK + Nav2** real-boat chain (no simulator, no thrust), `./real_boat.sh`
wraps `ros2 launch my_mapping_launcher real_boat_system.launch.py` — full setup and wiring in
`MAVLINK_RTK_SETUP.md` (Chinese). The RTK/MAVLink pieces are in `usv_navigation`:

- `mavlink_rtk_bridge.py` — minimal hand-rolled MAVLink subset over pyserial (no external
  mavlink dep): FC telemetry UART → `/gps/fix`, `/gps/heading` (ENU yaw = π/2 − compass),
  `/gps/status` (JSON diagnostics). Also uploads the path as a MAVLink mission when
  `mission_upload_enabled:=true`.
- `heading_to_imu.py` — fuses RTK absolute yaw with L1 IMU roll/pitch → `/gps/heading_imu`
  (stops publishing if RTK yaw goes stale >2.5 s).
- `geodetic_goal_planner.py` / `geodetic_path_publisher.py` / `target_to_goal.py` /
  `target_waypoint_serial.py` — path-only WGS84 ↔ UTM ↔ metric goal conversion and serial
  target input. **These compute/republish paths and never send `NavigateToPose` or thrust.**
- `usv_monitor/` — Qt5 dashboard for real-boat diagnostics; reads params and writes only the
  explicitly listed core params, never actuators.
...
- `experiments/` — trial runners and offline analysis: `run_static_obstacle_trial.sh`,
  `run_dynamic_ab_trial.sh`, `run_static_plan_trial.sh`, `run_sudden_obstacle_trial.sh`,
  `compare_navigation_trials.py` (`static` / `dynamic` subcommands), `record_nav_path.py`,
  `nav_trial_analysis.py`, `compute_ekf_ab_metrics.py`, plus probe/measure scripts.
...
- `report/` LIO + obstacle-filter improvements → `report2/` EKF vs Point-LIO A/B →
  ...
  `USV_SYSTEM_TECHNICAL_DOCUMENTATION.md` the system write-up). `report4/` is the sudden-obstacle
  (injected mid-cruise) avoidance experiment; its `CHANGES.md` logs two systematic bugs worth
  knowing: Nav2 `default_server_timeout`/`bt_loop_duration` are in **milliseconds**, and
  obstacle-injection coordinates must come from **Gazebo world truth**, not drifted LIO. Reports
  are written in Chinese; match that language when adding to them.
